...
- **景别有变化**：分镜模型逐句决定全景 / 中景 / 特写 —— 全景交代环境，特写落在情绪和结论上，而不是三十个镜头都用同一个机位距离。
- **片子会呼吸**：段落结尾插入气口（只剩 BGM，画面撑着不黑），BGM 在人声下压、在空档里抬起，最后一句念完画面再留 1.8 秒。
- **统一调色**：最上层一条滤镜轨铺满全片，把各自独立生成、色温会漂的图拉到同一个调子里。
- **字幕基线稳定**：换行的字幕会整体上抬，让底行基线保持不动，不会因为行数变化而上下跳。
...
| `SUBTITLE_EM_PX` | `9.8` | 换行估算用；字幕换行仍会上下飘时调这一个数。 |
...
| `ENDING_HOLD_SECONDS` | `1.8` | 最后一句之后画面留多久。 |
| `BGM_LIFT_VOLUME` | `0.20` | 空档里 BGM 抬到多少。 |
...
[字幕]            每句一段，带入场动画；气口里没有字幕
[BGM]    人声下压到 0.10，气口和结尾抬到 0.20；整轨循环，首尾淡入淡出
[调色]   一条滤镜铺满全片
```

气口出现在分镜模型标记为段落结尾的句子之后，所以八个自然段的稿子会有七处呼吸点，而不是每句都停。
